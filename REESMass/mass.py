...
def __compute_projected_face_integral(mesh, fh, X, Y, Z):
    ...
    for he in mesh.fh(fh):
        v0 = MESH.get_vertex_coords(mesh, mesh.from_vertex_handle(he))
        v1 = MESH.get_vertex_coords(mesh, mesh.to_vertex_handle(he))
...
def __rotate_to_model_space_orientation(prop):
    ...
    :param prop:   REESMass properties given in some "model space"
...
def compute_mass_properties(mesh, density):
    prop = MassProperties()
    ...
    for fh in mesh.faces():
        ...
        (P1, Pa, Paa, Paaa, Pb, Pbb, Pbbb, Pab, Paab, Pabb) = __compute_projected_face_integral(mesh, fh, X, Y, Z)
        ...
        Faaa = k1 * Paaa
        Fbbb = k1 * Pbbb
        Fccc = -k4 * (nA * nA * nA * Paaa + 3.0 * nA * nA * nB * Paab + 3.0 * nA * nB * nB * Pabb + nB * nB * nB * Pbbb + 3.0 * w * (nA * nA * Paa + 2.0 * nA * nB * Pab + nB * nB * Pbb) + w * w * (3.0 * (nA * Pa + nB * Pb) + w * P1))
        Faab = k1 * Paab
        Fbbc = -k2 * (nA * Pabb + nB * Pbbb + w * Pbb)
        Fcca = k3 * (nA * nA * Paaa + 2.0 * nA * nB * Paab + nB * nB * Pabb + w * (2.0 * (nA * Paa + nB * Pab) + w * Pa))
...
def update_inertia_tensor(R, Bv):
    ...
    #W = R.dot(I.dot(RT))  # TODO Kenny 2017-02-12: We can hand optimize this to exploit zero-pattern of Bv
    W = M3.zero()
