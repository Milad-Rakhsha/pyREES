...
def create_ground(engine,r, q, density, material_name, visual_material_name):
    shape_name = generate_unique_name(engine, 'ground_shape')
    create_box_shape(engine, shape_name, 100.0, 1.0, 100.0)
    body_name = generate_unique_name(engine, 'ground_body')
    ...
    q_g = Q.prod(q, q_l)
    set_position(engine, body_name, r_g, True)
    set_orientation(engine, body_name, q_g, True)
    ...
    set_visual_material(engine, body_name, visual_material_name)

def create_pillar(engine,
                  r,
                  q,
                  width,
                  height,
    ...
    shape_name = generate_unique_name(engine, 'socle_shape')
    create_box_shape(engine, shape_name, pillar_width, base_height, pillar_depth)
    body_name = generate_unique_name(engine, 'socle_body')
    ...
    shape_name = generate_unique_name(engine, 'abacus_shape')
    create_box_shape(engine, shape_name, pillar_width, top_height, pillar_depth)
    body_name = generate_unique_name(engine, 'abacus_body')
    body_names.append(body_name)
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    ...
    shape_name = generate_unique_name(engine, 'base_shape')
    create_conical_shape(engine, shape_name, pillar_base_radius, pillar_radius_start, cap_height)
    body_name = generate_unique_name(engine, 'base_body')
    body_names.append(body_name)
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    ...
    shape_name = generate_unique_name(engine, 'echinus_shape')
    create_conical_shape(engine, shape_name, pillar_radius_stop, pillar_top_radius, cup_height)
    body_name = generate_unique_name(engine, 'echinus_body')
    body_names.append(body_name)
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    ...
    for stone in range(pillar_stones):
        ...
        shape_name = generate_unique_name(engine, 'shaft_stone')
        create_conical_shape(engine, shape_name, bottom_radius, top_radius, dy)
        body_name = generate_unique_name(engine, 'shaft_body')
        body_names.append(body_name)
        create_rigid_body(engine, body_name)
        connect_shape(engine, body_name, shape_name)
...
def create_arch(engine,
    ...
    if (arch_stones % 2) == 0:
        raise RuntimeWarning('Arch stones was an even number, so no key stone will be created')
    if density <= 0.0:
        raise RuntimeWarning('Density must be positive')
    if pier_height <= 0.0:
        raise RuntimeWarning('arch height must be larger than half arch width, height > width/2.0')

    body_names = []

    shape_name = generate_unique_name(engine, 'pier_brick_shape')
    create_box_shape(engine, shape_name, pier_brick_width, pier_brick_height, pier_brick_depth)

    for stone in range(pier_stones):
        height = pier_brick_height / 2.0 + stone * pier_brick_height

        body_name = generate_unique_name(engine, 'left_pier_body')
        body_names.append(body_name)
        ...
        body_name = generate_unique_name(engine, 'right_pier_body')
        body_names.append(body_name)
    ...
    shape_name = generate_unique_name(engine, 'arch_brick_shape')
    ...
    create_cuboid_shape(engine, shape_name, p0, p1, p2, p3, p4, p5, p6, p7)

    for stone in range(arch_stones):
        body_name = generate_unique_name(engine, 'arch_body')
        body_names.append(body_name)
...
def create_dome(engine,
    ...
    for l in range(layers):
        theta = 0.0
        phi = d_phi * l
        shape_name = generate_unique_name(engine,  'brick_shape')
        ...
        for s in range(segments):
            ...
            body_name = generate_unique_name(engine,  'body')
            body_names.append(body_name)
...
def create_tower(engine,
    ...
    shape_name = generate_unique_name(engine, 'brick_shape')
    if use_cubes:
        brick_width = 2.0 * (sin(delta_phi / 2.0) / sin(pi / 2.0 - delta_phi / 2.0)) * inner_radius * safety_factor
        create_box_shape(engine, shape_name, brick_width, brick_height, brick_depth)
    else:
        brick_inner_half_width = (
                                 sin(delta_phi / 2.0) / sin(pi / 2.0 - delta_phi / 2.0)) * inner_radius * safety_factor
        brick_outer_half_width = (
                                 sin(delta_phi / 2.0) / sin(pi / 2.0 - delta_phi / 2.0)) * outer_radius * safety_factor
...
def create_colosseum(engine,
    ...
    arch_width = 2.0 * (sin(delta_phi / 2.0) / sin(pi / 2.0 - delta_phi / 2.0)) * inner_radius * safety_factor
    ...
    for i in range(I):
        for j in range(J):
            for k in range(K):
                ...
                connect_shape(engine, body_name, shape_name)
                set_position(engine, body_name, r_g, False)
                set_orientation(engine, body_name, q_g, False)
...
def create_funnel(engine,
    ...
    shape_names = []
    shape_name = generate_unique_name(engine, 'jack')
    create_shape_from_obj(engine, shape_name, 'resources/objs/jack.obj')
    resize_shape_to_unit_box(engine, shape_name, False)
    s = min(width / I, height / J, depth / K)*0.9  # The 0.9 scaling ensure some padding to avoid initial contact
    scale_shape(engine, shape_name, s, s, s)
    shape_names.append(shape_name)

    r = V3.make(-width/2.0, 3.0*funnel_height, -depth/2.0)
    q = Q.identity()
    body_names = create_grid(engine,
                             r,
                             q,
                             shape_names,
                             width,
    ...
    shape_name = generate_unique_name(engine, 'funnel')
    create_shape_from_obj(engine, shape_name, 'resources/objs/funnel.obj')
    resize_shape_to_unit_box(engine, shape_name, True)
    extends = compute_shape_extends(engine, shape_name, True)
    scale_shape(engine, shape_name,
    ...
    q = Q.Rx(pi)
    set_position(engine, body_name, r, True)
    set_orientation(engine, body_name, q, True)
    set_body_type(engine, body_name, 'fixed')
    set_body_active(engine, body_name, True)
    set_body_material(engine, body_name, material_name)
    set_mass_properties_from_shape(engine, body_name, density)
...
def create_glasses(engine,
    ...
    shape_names = []
    shape_name = generate_unique_name(engine, 'small_glass')
    create_shape_from_obj(engine, shape_name, 'resources/objs/glass.obj')
    resize_shape_to_unit_box(engine, shape_name, False)
    s = min(width / I, height / J, depth / K)*0.9  # The 0.9 scaling ensure some padding to avoid initial contact
    scale_shape(engine, shape_name, s, s, s)
    shape_names.append(shape_name)
    ...
    shape_name = generate_unique_name(engine, 'glass')
    create_shape_from_obj(engine, shape_name, 'resources/objs/glass.obj')
    resize_shape_to_unit_box(engine, shape_name, True)
    extends = compute_shape_extends(engine, shape_name, True)
    scale_shape(engine, shape_name,
...
def create_poles(engine,
    ...
    shape_names = []
    obj_files = ['resources/objs/cow.obj', 'resources/objs/dragon.obj', 'resources/objs/knot.obj', 'resources/objs/lamp.obj', 'resources/objs/propella.obj', 'resources/objs/shuttle.obj']

    for file in obj_files:
        shape_name = generate_unique_name(engine, 'small_obj_shape')
        create_shape_from_obj(engine, shape_name, file)
        resize_shape_to_unit_box(engine, shape_name, False)
    ...
    shape_name = generate_unique_name(engine, 'pole')
    create_cylinder_shape(engine, shape_name, pole_radius, pole_height)
...
def create_temple(engine,
    ...
    if (K_pillars-1) % 3 != 0:
        raise RuntimeError('K pillars must be equal to 3*n+1 for a given positive integer n')
    ...
    shape_name = generate_unique_name(engine, 'bottom_step_shape')
    create_box_shape(engine, shape_name, temple_width+6*pillar_width, stairs_height, temple_depth+6*pillar_depth)
    body_name = generate_unique_name(engine, 'bottom_step_body')
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    set_position(engine, body_name, V3.make(0.0, stairs_height*0.5, 0.0), True)
    set_orientation(engine, body_name, Q.identity(), True)
    body_names.append(body_name)

    shape_name = generate_unique_name(engine, 'middle_step_shape')
    create_box_shape(engine, shape_name, temple_width+4*pillar_width, stairs_height, temple_depth+4*pillar_depth)
    body_name = generate_unique_name(engine, 'middle_step_body')
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    set_position(engine, body_name, V3.make(0.0, stairs_height*1.5, 0.0), True)
    set_orientation(engine, body_name, Q.identity(), True)
    body_names.append(body_name)

    shape_name = generate_unique_name(engine, 'top_step_shape')
    create_box_shape(engine, shape_name, temple_width+2*pillar_width, stairs_height, temple_depth+2*pillar_depth)
    body_name = generate_unique_name(engine, 'top_step_body')
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    set_position(engine, body_name, V3.make(0.0, stairs_height*2.5, 0.0), True)
    set_orientation(engine, body_name, Q.identity(), True)
    body_names.append(body_name)

    shape_name = generate_unique_name(engine, 'interior_step_shape')
    create_box_shape(engine, shape_name, temple_width-2.5*pillar_width, stairs_height, temple_depth-2.5*pillar_depth)
    body_name = generate_unique_name(engine, 'interior_step_body')
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    set_position(engine, body_name, V3.make(0.0, stairs_height*3.5, 0.0), True)
    set_orientation(engine, body_name, Q.identity(), True)
    body_names.append(body_name)
    ...
    for i in range(I_pillars):
        ...
        q_g = Q.identity()
        names = create_pillar(engine, r_g, q_g, pillar_width, pillar_height, pillar_depth, pillar_stones, density, material_name, visual_material_name)
        body_names.extend(names)
    ...
    for k in range(1, K_pillars-1):
        ...
        q_g = Q.identity()
        names = create_pillar(engine, r_g, q_g, pillar_width, pillar_height, pillar_depth, pillar_stones, density,
                              material_name, visual_material_name)
        body_names.extend(names)
    ...
    for k in range(3, K_pillars-3, 3):
        ...
        for i in range(1,I_pillars-1):
            ...
            r_g = V3.make(x, y, z)
            q_g = Q.identity()
            names = create_pillar(engine, r_g, q_g, pillar_width, pillar_height-stairs_height, pillar_depth, pillar_stones, density,
                                  material_name, visual_material_name)
            body_names.extend(names)

    # Create epistyle (brick layer ontop of pillars)
    epistyle_height = 2.0*stairs_height
    shape_name = generate_unique_name(engine, 'regular_epistyle_brick_shape')
    create_box_shape(engine, shape_name, 2.0*pillar_width, epistyle_height, pillar_depth)
    ...
    for k in range(0, K_pillars, 3):
        ...
        for i in range(1, I_pillars-2):
            ...
            r_g = V3.make(x, y, z)
            q_g = Q.identity()
            body_name = generate_unique_name(engine, 'epistyle_brick_body')
            create_rigid_body(engine, body_name)
            connect_shape(engine, body_name, shape_name)
            set_position(engine, body_name, r_g, True)
            set_orientation(engine, body_name, q_g, True)
    ...
    shape_name = generate_unique_name(engine, 'corner_epistyle_brick_shape')
    ...
    shape_name = generate_unique_name(engine, 'wedge_epistyle_brick_shape')
    ...
    create_convex_shape(engine, shape_name, [p0, p1, p2, p3, p4, p5, p6, p7, p8, p9])
    ...
    shape_name = generate_unique_name(engine, 'junction_epistyle_brick_shape')
    ...
    shape_name_regular = generate_unique_name(engine, 'gable_brick_shape')
    ...
    create_cuboid_shape(engine, shape_name_regular, p0, p1, p2, p3, p4, p5, p6, p7)

    shape_name_edge = generate_unique_name(engine, 'gable_edge_brick_shape')
    ...
    create_convex_shape(engine, shape_name_edge, [p0, p1, p2, p3, p4, p5])
    ...
    for k in range(0, K_pillars, 3):
        ...
        for j in range(I_pillars-1):
            for i in range(I_pillars-j):
                ...
                x_brick = -temple_width/2.0 + pillar_width/2.0 + i*gable_brick_width + j*gable_brick_width*0.5
                y_brick = 3.0*stairs_height + pillar_height + epistyle_height + gable_brick_height/2.0 + j*gable_brick_height
                ...
                body_name = generate_unique_name(engine, 'gable_brick_body')
                create_rigid_body(engine, body_name)
                connect_shape(engine, body_name, shape_name)
                set_position(engine, body_name, r_g, True)
                set_orientation(engine, body_name, q_g, True)
    ...
    shape_name_left_long = generate_unique_name(engine, 'long_left_plank_shape')
    x_left_bottom = 0.0
    ...
    create_cuboid_shape(engine, shape_name_left_long, p0, p1, p2, p3, p4, p5, p6, p7)

    shape_name_left_short = generate_unique_name(engine, 'short_left_plank_shape')
    x_left_bottom = 0.0
    x_right_bottom = pillar_width / 2.0
    x_left_top = pillar_width
    x_right_top = pillar_width * 1.5
    ...
    p6 = V3.make(x_right_top, y_top, z_back)
    p7 = V3.make(x_left_top, y_top, z_back)
    create_cuboid_shape(engine, shape_name_left_short, p0, p1, p2, p3, p4, p5, p6, p7)

    shape_name_right_long = generate_unique_name(engine, 'long_right_plank_shape')
    x_right_bottom = 0.0
    ...
    p6 = V3.make(x_right_top, y_top, z_back)
    p7 = V3.make(x_left_top, y_top, z_back)
    create_cuboid_shape(engine, shape_name_right_long, p0, p1, p2, p3, p4, p5, p6, p7)

    shape_name_right_short = generate_unique_name(engine, 'short_right_plank_shape')
    x_right_bottom = 0.0
    x_left_bottom = -pillar_width / 2.0
    x_right_top = -pillar_width
    x_left_top = -pillar_width * 1.5
    ...
    p6 = V3.make(x_right_top, y_top, z_back)
    p7 = V3.make(x_left_top, y_top, z_back)
    create_cuboid_shape(engine, shape_name_right_short, p0, p1, p2, p3, p4, p5, p6, p7)
    ...
    for k in range(0, K_pillars-1, 3):
        ...
        for i in range(I_pillars-1):
            y_plank = 3.0 * stairs_height + pillar_height + epistyle_height + i * gable_brick_height
            q_g = Q.identity()

            x_left_plank = -temple_width / 2.0 + i * gable_brick_width* 0.5
            r_g = V3.make(x_left_plank, y_plank, z_plank)
            body_name = generate_unique_name(engine, 'plank_body')
            create_rigid_body(engine, body_name)
            connect_shape(engine, body_name, shape_name_left)
            set_position(engine, body_name, r_g, True)
            set_orientation(engine, body_name, q_g, True)
            body_names.append(body_name)
            ...
            body_name = generate_unique_name(engine, 'plank_body')
            create_rigid_body(engine, body_name)
            connect_shape(engine, body_name, shape_name_right)
            set_position(engine, body_name, r_g, True)
            set_orientation(engine, body_name, q_g, True)
            body_names.append(body_name)

    shape_name = generate_unique_name(engine, 'ridge_brick_shape')
    ...
    create_convex_shape(engine, shape_name, [p0, p1, p2, p3, p4, p5])

    for k in range(K_pillars*2-1):
        z = temple_depth/2.0 - (k+1)*pillar_depth
        x = 0
        y = 3.0*stairs_height+pillar_height+epistyle_height+(I_pillars-1)*gable_brick_height
        r = V3.make(x, y, z)
        q = Q.identity()
        body_name = generate_unique_name(engine, 'ridge_body')
        create_rigid_body(engine, body_name)
        connect_shape(engine, body_name, shape_name)
        set_position(engine, body_name, r, True)
        set_orientation(engine, body_name, q, True)
...
def create_sandbox(engine,
    ...
    shape_names = []
    for n in range(20):
        shape_name = generate_unique_name(engine, 'grain')
        points = [V3.rand(-1.0,1.0) for _ in range(20)]
        create_convex_shape(engine, shape_name, points)
        resize_shape_to_unit_box(engine, shape_name, False)
        s = min(box_width / I_grains, box_height / J_grains, box_depth / K_grains)*0.9  # The 0.9 scaling ensure some padding to avoid initial contact
        scale_shape(engine, shape_name, s, s, s)
        shape_names.append(shape_name)
    ...
    shape_name = generate_unique_name(engine, 'box_left')
    create_box_shape(engine, shape_name, thickness, box_height, box_depth)
    body_name = generate_unique_name(engine, 'body_left')
    body_names.append(body_name)
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    ...
    shape_name = generate_unique_name(engine, 'box_right')
    create_box_shape(engine, shape_name, thickness, box_height, box_depth)
    body_name = generate_unique_name(engine, 'body_right')
    body_names.append(body_name)
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    r = V3.make( box_width/2.0 + thickness/2.0, box_height/2.0 + thickness, 0.0)
    ...
    shape_name = generate_unique_name(engine, 'box_front')
    create_box_shape(engine, shape_name, thickness*2.0 + box_width, box_height, thickness)
    body_name = generate_unique_name(engine, 'body_front')
    body_names.append(body_name)
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    ...
    shape_name = generate_unique_name(engine, 'box_back')
    create_box_shape(engine, shape_name, thickness * 2.0 + box_width, box_height, thickness)
    body_name = generate_unique_name(engine, 'body_back')
    body_names.append(body_name)
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
    r = V3.make(0.0, box_height / 2.0 + thickness, -box_depth / 2.0 - thickness / 2.0)
    ...
    shape_name = generate_unique_name(engine, 'base')
    create_box_shape(engine, shape_name, thickness * 2.0 + box_width, thickness, box_depth + 2.0*thickness)
    body_name = generate_unique_name(engine, 'base')
    body_names.append(body_name)
    create_rigid_body(engine, body_name)
    connect_shape(engine, body_name, shape_name)
