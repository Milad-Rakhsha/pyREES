...
float wire_edge_factor(float thickness)
{
...
void main()
{
  ...
  {
    vec3 color = compute_light_intensity(lights[i], material, v_position_eye, v_normal_eye);
    accum = accum + color;
  }

  frag_color = vec4( mix(material.wire_color, accum, wire_edge_factor(material.wire_thickness)), 1.0 );
}
