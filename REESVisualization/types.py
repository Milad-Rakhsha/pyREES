...
class Shader:
    ...
    def __init__(self, filename, type):
        ...
        #weakref.finalize(self, print, 'Shader', id(self))
...
class ShaderProgram:
    ...
    def __init__(self, shaders):
        ...
        #weakref.finalize(self, print, 'ShaderProgram', id(self))
...
class VAO:
    ...
    def __init__(self):
        ...
        #weakref.finalize(self, print, 'VAO', id(self))
...
class VBO:
    ...
    def __init__(self, vertices, indices, make_static=True, make_triangles=True):
        self.is_triangles =  make_triangles
        self.is_static = make_static
        self.vertex_count = vertices.size / 6
        ...
        #weakref.finalize(self, print, 'VBO', id(self))

    def update(self, vertices):
        if self.vertex_count != vertices.size / 6:
    ...
    def draw(self):
        ...
        #     offset             # Element array buffer offset
        #     )
...
class ShapeNode:
    ...
    def __init__(self, program, vao, vbo):
        ...
        self.vao.unbind()
        #weakref.finalize(self, print, 'RenderSetup', id(self))
...
class ContactPointsInstanceNode:

    def __init__(self, shape_node, material, scale=0.5, engine=None):
        self.shape_node = shape_node
        self.material = material
        self.model_matrix = np.identity(4, dtype=np.float32)
        self.engine = engine
    ...
    def update_from_contact_point(self, cp):

        up = V3.j()
        n = cp.n
        p = cp.p

        cos_theta = np.dot(up, n)

        axis = V3.i()
        if fabs(cos_theta) < 0.99:
            axis = V3.cross(up, n)

        radians = acos(cos_theta)

        self.model_matrix[0:3, 0:3] = np.dot(M3.diag(self.scale, self.scale, self.scale), M3.Ru(radians, axis))
        self.model_matrix[0:3, 3] = p

    def draw(self):
        for cp in self.engine.contact_points:
            self.update_from_contact_point(cp)
            self.shape_node.program.set_uniform_matrix4('model_matrix', self.model_matrix)
            self.shape_node.program.set_uniform_material(self.material)
            self.shape_node.draw()


class JointConnectorsInstanceNode:

    def __init__(self, shape_node, sokcet_material, plug_material, scale=0.5, engine=None):
        self.shape_node = shape_node
        self.plug_material = plug_material
        self.socket_material = sokcet_material
        self.model_matrix = np.identity(4, dtype=np.float32)
        self.engine = engine
        self.scale = scale

    def update_model_matrix(self, p, n):
    ...
    def draw(self):
        for joint in self.engine.joints.values():
            socket = joint.socket

            p = socket.get_world_anchor()
            n = socket.get_world_axis_3()
            self.update_model_matrix(p, n)

            self.shape_node.program.set_uniform_matrix4('model_matrix', self.model_matrix)
            self.shape_node.program.set_uniform_material(self.socket_material)
            self.shape_node.draw()

            plug = joint.plug
            p = plug.get_world_anchor()
            n = plug.get_world_axis_3()
            self.update_model_matrix(p, n)

            self.shape_node.program.set_uniform_matrix4('model_matrix', self.model_matrix)
            self.shape_node.program.set_uniform_material(self.plug_material)
            self.shape_node.draw()
